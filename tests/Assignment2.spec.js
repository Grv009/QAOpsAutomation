...
const email = 'g.sarkar22@gmail.com';
...
async function loginAndGoToBooking(page) {
    ...
    await Promise.all([
        page.waitForURL('https://eventhub.rahulshettyacademy.com/'),
...
async function fillBookingForm(page) {
    ...
    await page.getByLabel('Full Name').fill('Gourav Sarkar');
...
test('Single ticket booking is eligible for refund', async ({ page }) => {
    ...
    const bookingRef = (await page.locator("//span[contains(@class,'font-mono font-bold')]").innerText()).trim();
